...
void env_set_nice(struct Env* e, int nice_value)
{
	...
	fixed_point_t recent4 = fix_unscale(e->recent_cpu, 4);
	fixed_point_t first_minus = fix_sub(fix_int(PRI_MAX), recent4);
	fixed_point_t second_minus = fix_sub(first_minus, nice2);
	if(PRI_MAX - e->priority != fix_trunc(second_minus))
	{
		e->priority = fix_trunc(second_minus);//what?
		int queue_idx = PRI_MAX - e->priority; ///when pr = 63(max) -> this is queue [0]
		enqueue(&env_ready_queues[queue_idx], e);
	}
}
