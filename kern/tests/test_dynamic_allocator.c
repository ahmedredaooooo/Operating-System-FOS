...
void test_realloc_block_FF()
{
	...
	*(endVAs[idx]) = idx ;
	...
	// OUR CASES START
		/*
		 * increased size:
		 * 		test that next block is free but not sufficient
		 * 			prev block is free
		 * 			prev block is not free
		 * 		test that next block is not free
		 * 							X
		 * 		new location split
		 * 		new location no split
		 * same size:
		 *
		 * decreased size:
		 * 		 next block is not full (coalesce)
		 */

		// SAME SIZE CHECK
		cprintf("5: Test calling realloc with same size.\n\n");
		{
			...
			//check next block that "not affected"
			struct BlockMetaData *newBlkMetaData = (struct BlockMetaData *)(va + new_size);
			expected_size = allocSizes[1];
			if (newBlkMetaData->size != expected_size || !newBlkMetaData->is_free)
			{
				cprintf("test_realloc_block_FF #15.4: WRONG REALLOC! next block is not correct... check it!. Expected %d, Actual %d\n", expected_size, newBlkMetaData->size);
				panic("WRONG REALLOC");
			}
			//check content of reallocated block
			if (*(startVAs[blockIndex]) != blockIndex || *(midVAs[blockIndex]) != blockIndex ||	*(endVAs[blockIndex]) != blockIndex)
		...
		// TEST that next block is free but not sufficient & the previous block is free too
		cprintf("7: TEST that next block is free but not sufficient & the previous block is free too.\n\n");
		{
			blockIndex = 5 * allocCntPerSize - 1; /*2KB*/
			realloc_block_FF(startVAs[blockIndex - 1], 0); // now current block have free block of size 2KB before it
															//and free block of size 2*16B after it
			old_size = allocSizes[4] /*2KB*/;
			new_size = old_size + 1*kilo - sizeOfMetaData();

			va = realloc_block_FF(startVAs[blockIndex], new_size);
			expected_va = startVAs[0];
			//check return address
			...
			//check next block that "not affected"
			struct BlockMetaData *newBlkMetaData = (struct BlockMetaData *)(expected_va + new_size);
			expected_size = 1*kilo;
			if (newBlkMetaData->size != expected_size || newBlkMetaData->is_free != 1)
			{
				cprintf("test_realloc_block_FF #17.4: WRONG REALLOC! newly created block is not correct... check it!. Expected %d, Actual %d\n", expected_size, newBlkMetaData->size);
				panic("WRONG REALLOC");
			}
			if (get_block_size(startVAs[blockIndex - 1]) != 4 * kilo + 2 * sizeOfMetaData())
			{
				cprintf("test_realloc_block_FF #17.6: WRONG REALLOC! size of the old block is not correct. Expected %d, Actual %d\n", 4 * kilo + 2 * sizeOfMetaData(), get_block_size(startVAs[blockIndex - 1]));
				panic("WRONG REALLOC");
			}
		}
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
