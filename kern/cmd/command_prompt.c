...
int process_command(int number_of_arguments, char** arguments)
{
	...
	{
		if (strlen(arguments[0]) > strlen(commands[i].name)) continue;
		// now : we sure that arguments[0] length <= current command length

		// check name equivalence
		if (!strcmp(arguments[0], commands[i].name))
		{
			LIST_INIT(&foundCommands);
			// check arg equivalence
			if ((!~commands[i].num_of_args && number_of_arguments > 1) || number_of_arguments - 1 == commands[i].num_of_args)
				return i;
